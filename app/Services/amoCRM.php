...
class amoCRM
{
    ...
    public function __construct()
    {
        ...
        //$oauth = $this->amoApi->fetchAccessToken(env('AMO_AUTH_CODE'));dd($oauth);
    ...
    private function searchContact(Client $client)
    {
        ...
        $contact = $contacts->first() ?? null;//TODO тернарный
    ...
    {
        Log::info(__METHOD__.' record id : '.$record->id.' status_id : '.$status_id);
    ...
    public function createAbonement(Client $client, Abonement $abonement)
    {
        ...
        $lead->cf('Фактический баланс')->setValue($abonement->sale);
    ...
    public function updateLeadNoPay(Record $record)
    {
        if($record->lead_id) {
...
//            $lead->cf('roistat')->setValue('yclients_'.Record::getFilial($record->company_id));
            ...
            try {
                ...
                Log::error(__METHOD__.' : OK : '.$result);
            }
    ...
    public function updateStatus($lead, int $status_id = null)
    {
        Log::info(__METHOD__.' lead id : '.$lead->id.' status_id : '.$status_id);
    ...
    public function searchOrCreate(Client $client, Record $record)
    {
        if(!$record->lead_id) {
            ...
            if(!$lead)
                $lead = $this->createLead($client, $record, self::pipelineHelper(env('FIRST_PIPELINE'), $record));
            ...
            if($lead)
                ...
                Log::info(__METHOD__.' id сделки в бд не найден в амо : '.$record->lead_id);
        }
    ...
    {
        Log::info(__METHOD__.' id : '.$id.' record_id : '.$record->id);
    ...
    {
        Log::info(__METHOD__.' client id : '.$client->id.' pipeline_id : '.$pipeline_id);
